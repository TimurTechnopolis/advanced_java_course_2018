...
public class CustomString implements CharSequence, Serializable { // Имплементим нужный нам интерфейс
    ...
    public char charAt(int index) {
        return m[((offset + index) / (m[0].length))][((offset + index) % (m[0].length))]; // Получаем символ исходя из нашего индекса со сдвигом вправо (так как начальное значение - offset)
    }
    ...
    public String toString() {
        ...
        int tmp = m[0][0] == '\uFEFF' ? 1 : 0; // В начале файла стоит \uFEFF, поэтому пропускаем его при выводе строки
